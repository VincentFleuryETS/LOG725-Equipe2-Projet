...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public bool ResolveActionAndBinding(out InputAction action, out int bindingIndex)
        {
            ...
            if (bindingIndex == -1)
            {
                Debug.LogError($"Impossible de trouver un Binding avec ID '{bindingId}' sur '{action}'", this);
                return false;
            }
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            if (action.bindings[bindingIndex].isPartOfComposite)
                partName = $"Direction '{action.bindings[bindingIndex].name}'. ";
            ...
            if (m_RebindText != null)
            {
                var text = !string.IsNullOrEmpty(m_RebindOperation.expectedControlType)
                    ? $"{partName}En attente d'input de type '{m_RebindOperation.expectedControlType}'..."
                    : $"{partName}En attente d'input...";
